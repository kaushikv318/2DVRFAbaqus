...
!************************************************************************
      ...
      parameter(numElem=17177,offset=17177)
...
!****************************************************************************
     ...
     +               ,sig_taun,endenn,Hs)
         ...
         !
            ...
            Y_t = svars(1+jj)
            !sig_taup1(1,1) = svars(2+jj)
            !sig_taup1(2,2) = svars(3+jj)
            !sig_taup1(1,2) = svars(4+jj)
            end if
            ...
            !sig_taup1(1,1) = sig_taup(1,1)
            svars(3+jj) = sig_taup(2,2)
            !sig_taup1(2,2) = sig_taup(2,2)
            svars(4+jj) = sig_taup(1,2)
            !sig_taup1(1,2) = sig_taup(1,2)
            endif
            !write(6,*)'svarj',svars(jj+1),endenp,Y_t,kinc
     ...
     +           *matmul(Transpose(Nmat),Nmat) 
         ...
         globalSdv(elemPt,intPt,4) = Smatp(1,1)
         globalSdv(elemPt,intPt,5) = Smatp(2,2)
         globalSdv(elemPt,intPt,6) = sig_tau(1,2)
         globalSdv(elemPt,intPt,7) = Smatp(1,2)
         globalSdv(elemPt,intPt,8) = phi_tau_i
         globalSdv(elemPt,intPt,9) = Y_t
         globalSdv(elemPt,intPt,10) = endenp
